...
public class Server extends UnicastRemoteObject
    ...
    public void get (String file) throws IOException, FileNotFoundException, RemoteException {
        if (!valid(file)) {
        	...
        	BufferedOutputStream bufferedOutputStream = new BufferedOutputStream(xfer.getOutputStream());
    ...
    public void put (String file) throws IOException, FileNotFoundException, RemoteException {
    	...
    	try {
			if (mode == Mode.ACTIVE) {
				...
				Socket socket = new Socket(clientSocket.getAddress(),clientSocket.getPort());
				BufferedInputStream bufferedInputStream = new BufferedInputStream(socket.getInputStream());
				...
				FileOutputStream fileOutputStream = new FileOutputStream(path() + file);
				new Thread(new PutThread(dataChan, fileOutputStream)).start();
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
